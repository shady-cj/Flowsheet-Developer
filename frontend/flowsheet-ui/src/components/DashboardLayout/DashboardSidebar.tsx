...
const DashboardSidebar = () => {
  ...
  return (
    <div className="w-[22%] shrink-0 bg-white border-r">
        <section className="w-full">
          <div className="h-[13.5rem] xl:h-[14.5rem] border-b border-solid border-[#E6E6E6] pt-8 xl:pt-12">
            <div className="px-6 w-full flex flex-col gap-y-4 ">
              <Link href="/dashboard" className={`w-full inline-flex ${type === null ? "bg-normalBlueVariant": "bg-grayVariant"} flex-row items-center gap-x-3 px-4 py-2.5 rounded-lg`}>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" className="w-4 h-4 xl:w-6 xl:h-6" viewBox="0 0 48 48" fill="none">
                ...
                <span className={`font-normal text-sm xl:text-base ${type === null ? "text-[#F5F5F5]": "text-text-black-2"}`}>Home</span>
              </Link>
              <Link href="/dashboard?f=recents" className={`w-full inline-flex ${type === "recents"? "bg-normalBlueVariant": "bg-grayVariant"} flex-row items-center gap-x-3 px-4 py-2.5 rounded-lg`}>
                <svg width="24" height="24" className="w-4 h-4 xl:w-6 xl:h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g id="clock-01">
                ...
                <span className={`font-normal text-sm xl:text-base ${type === "recents" ? "text-[#F5F5F5]": "text-text-black-2"}`}>Recents</span>
              </Link>
              <Link href="/dashboard?f=starred" className={`w-full inline-flex ${type === "starred"? "bg-normalBlueVariant": "bg-grayVariant"} flex-row items-center gap-x-3 px-4 py-2.5 rounded-lg`}>
                ...
                <span className={`font-normal text-sm xl:text-base ${type === "starred" ? "text-[#F5F5F5]": "text-text-black-2"}`}>Starred</span>
              </Link>

            </div>
          </div>
            
          <div className="pt-8 xl:pt-12 px-6">
            <div className="relative text-sm xl:text-base">
              ...
              <select name="loadData" onChange={fetchData} id="" ref={selectType} className="w-full text-text-black focus:border-[#CCC] border border-[#CCCCCC] py-2 xl:py-3 px-7 rounded-xl">
            ...
            <div className="py-5 px-3 flex flex-col gap-5 text-sm xl:text-base">
              {loading ? <Loader fullScreen={false} offsetHeightClass="h-[150px]" color="black"/> : data?.length ? data.map((item)=>{
                ...
                return (<Link href={link} key={item.id} className="flex justify-between text-[#4D4D4D] hover:text-text-black-2 items-center">
                  <h2>{item.name}</h2>
                  <Image src={arrowRight} height={10} width={10} className="w-[8px] h-[8px] xl:w-[10px] xl:h-[10px]" alt="arrow right"/>
                </Link>)
              }):""}
            </div>
