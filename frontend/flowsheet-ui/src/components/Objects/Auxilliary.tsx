...
const Auxilliary = ({auxilliary}: {auxilliary: AuxilliaryImageObjectType}) => {
  return (
    <div className="flex flex-col items-center gap-2 h-full" key={auxilliary.id}>
        <div id={auxilliary.id} data-object-name={auxilliary.name} data-object-type={"Auxilliary"} data-object-type-variant={auxilliary.type} className="flex-1 flex items-center justify-center objects bg-transparent cursor-grab" draggable={true} onDragStart={(e)=>{e.dataTransfer.setData("elementId", (e.target as HTMLDivElement).id)}}>
