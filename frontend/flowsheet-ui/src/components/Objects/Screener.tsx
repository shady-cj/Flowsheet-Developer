...
const Screener = ({screener}: {screener: genericImageObjectType}) => {
  return (
    <div className="flex flex-col justify-between items-center gap-2 h-full" key={screener.id}>
        <div id={screener.id} data-object-name={screener.name} data-object-type={"Screener"} className="flex-1 flex items-center justify-center objects bg-transparent cursor-grab" draggable={true} onDragStart={(e)=>{e.dataTransfer.setData("elementId", (e.target as HTMLDivElement).id)}}>
